...
export class EffectsManager {
    ...
    createBallTrail(game) {
        ...
        const trailMaterial = new THREE.LineBasicMaterial({ color: 0xff6600, linewidth: 1, transparent: true, opacity: 0.7 });
    ...
    playFireEffect(game, position) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            if (!game._sharedFireGeo) game._sharedFireGeo = new THREE.BoxGeometry(1, 1, 1);
            ...
            particle.userData.velocity = new THREE.Vector3((Math.random() - 0.5) * 0.3, Math.random() * 0.2 + 0.1, (Math.random() - 0.5) * 0.3);
    ...
    playLightningEffect(game, from, to) {
        ...
        for (let i = 1; i < segments; i++) {
            ...
            points.push(new THREE.Vector3(baseX + (Math.random() - 0.5) * offset, baseY + (Math.random() - 0.5) * offset, baseZ + (Math.random() - 0.5) * offset));
        }
        points.push(new THREE.Vector3(to.x, to.y, to.z));
        for (let i = 0; i < points.length - 1; i++) {
            const start = points[i], end = points[i + 1];
            const length = start.distanceTo(end);
            const midX = (start.x + end.x) / 2, midY = (start.y + end.y) / 2, midZ = (start.z + end.z) / 2;
    ...
    updateParticles(game) {
        for (let i = game.particles.length - 1; i >= 0; i--) {
            ...
            if (particle.userData.life <= 0) {
                ...
                if (particle.geometry !== game._sharedFireGeo) particle.geometry.dispose();
    ...
    updateBallTrail(game) {
        ...
        if (points.length < 2) { game.ballTrailLine.visible = false; return; }
        const posAttr = game.ballTrailLine.geometry.attributes.position;
        const arr = posAttr.array;
        const len = Math.min(points.length, game.ballTrailMax);
        for (let i = 0; i < len; i++) { arr[i * 3] = points[i].x; arr[i * 3 + 1] = points[i].y; arr[i * 3 + 2] = points[i].z; }
    ...
    updateSlowMotion(game, delta) {
        if (game.slowMotionTimer > 0) {
            ...
            delta *= 0.2;
            if (game.bloomPass) { game.bloomPass.strength = 0.3; game.bloomPass.threshold = 0.6; game.bloomPass.radius = 0.2; }
            if (game.rgbShiftPass) game.rgbShiftPass.uniforms['amount'].value = 0.01;
        } else {
            if (game.bloomPass) { game.bloomPass.strength = 0.3; game.bloomPass.threshold = 0.6; game.bloomPass.radius = 0.2; }
            if (game.rgbShiftPass) game.rgbShiftPass.uniforms['amount'].value = 0.0;
        }
        return delta;
    }
    ...
    updateBallTrailPoints(game) {
        if (game.basketballBody && game.basketball) {
            const pos = new THREE.Vector3(game.basketballBody.position.x, game.basketballBody.position.y, game.basketballBody.position.z);
            game.ballTrail.push(pos);
            if (game.ballTrail.length > game.ballTrailMax) game.ballTrail.shift();
