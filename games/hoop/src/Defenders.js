...
export class Defenders {
    constructor(game) {
        ...
        game.exclamationCanvas.width = 128;
        game.exclamationCanvas.height = 128;
    ...
    initTestDefenders(game) {
        this.addDefender(game, -6, 3);
        this.addDefender(game, 6, 10);
    }
    ...
    addDefender(game, initialX, initialZ) {
        ...
        const body = new CANNON.Body({
            ...
            fixedRotation: true,
            allowSleep: false,
            material: game.defenderMaterial,
        ...
        const defender = {
            mesh: mesh,
            body: body,
            continuousStealTime: 0,
            pickupCooldown: 0,
            stealHistory: [],
            cooldown: 0,
            hasBall: false,
            hasTriggeredWhat: false
        };
    ...
    updateDefenders(game, delta) {
        ...
        let maxContinuousPct = 0;
        let maxCumulativePct = 0;
        let triggerRedThisFrame = false;

        if (game.exclamationRedTimer > 0) {
            game.exclamationRedTimer -= delta;
            triggerRedThisFrame = true;
        }
        ...
        if (activeOffender) {
            for (const def of game.defenders) {
                if (!def.hasBall) {
                    const dx = playerPos.x - def.body.position.x;
                    const dz = playerPos.z - def.body.position.z;
                    const distXZ = Math.sqrt(dx * dx + dz * dz);
                    if (distXZ <= 1.0 && !def.hasTriggeredWhat) {
                        game.sound.playWhat(game);
                        def.hasTriggeredWhat = true;
                    }
                }
            }
        }
        ...
        for (const def of game.defenders) {
            ...
            if (def.hasBall) {
                this._handleOffense(game, def, delta);
            } else if (game.ballAttached && def.cooldown <= 0) {
                this._handleStealAttempt(game, def, playerPos, delta, now);
                const defPos = def.body.position;
                const dx = playerPos.x - defPos.x;
                const dz = playerPos.z - defPos.z;
                const distXZ = Math.sqrt(dx * dx + dz * dz);

                if (distXZ <= 1.0) {
                    def.continuousStealTime += delta;
                    def.stealHistory.push({ time: now, delta: delta });
                } else {
                ...
                if (def.continuousStealTime >= 1.0 || accumulatedTime >= 1.5) {
                    this.handleStealSuccess(game, def);
                    triggerRedThisFrame = true;
                    game.exclamationRedTimer = 0.4;
                }
                ...
                if (distToOffender < 2.0) {
                    const pushX = def.body.position.x - activeOffender.body.position.x;
                    const pushZ = def.body.position.z - activeOffender.body.position.z;
                    ...
                    this._screenAndBlockPlayer(def, playerPos, delta);
                }
            } else {
                def.continuousStealTime = 0;
                if (!game.ballAttached) def.stealHistory = [];
                const distToBall = def.body.position.distanceTo(ballPos);
                this._moveDefenderTo(def, ballPos, delta, 0.85);
                if (distToBall < 1.0 && !game.defenderHoldingBall && def.pickupCooldown <= 0) {
                    this.defenderPickUpBall(game, def);
                }
            }
        ...
        if (game.exclamationSprite && game.playerModel) {
            game.exclamationSprite.position.set(playerPos.x, playerPos.y + 2.2, playerPos.z);
            game.ui.drawExclamationUI(game, maxContinuousPct, maxCumulativePct, triggerRedThisFrame);
        }
    }

    _handleOffense(game, def, delta) {
        const dx = game.opponentHoop.x - def.body.position.x;
        const dz = game.opponentHoop.z - def.body.position.z;
        const distXZ = Math.sqrt(dx * dx + dz * dz);

        let avoidX = 0, avoidZ = 0;
        for (const otherDef of game.defenders) {
            if (otherDef !== def) {
                const ox = def.body.position.x - otherDef.body.position.x;
                const oz = def.body.position.z - otherDef.body.position.z;
                const oDist = Math.sqrt(ox * ox + oz * oz);
                if (oDist < 2.5 && oDist > 0.1) {
                    avoidX += (ox / oDist) * (2.5 - oDist) * 1.5;
                    avoidZ += (oz / oDist) * (2.5 - oDist) * 1.5;
                }
            }
        }

        if (distXZ > 2.0) {
            const baseDirX = dx / distXZ;
            const baseDirZ = dz / distXZ;
            const finalDirX = baseDirX + avoidX;
            const finalDirZ = baseDirZ + avoidZ;
            const fLen = Math.sqrt(finalDirX * finalDirX + finalDirZ * finalDirZ);
            const speed = MOVE_SPEED * 1.2;
            def.body.velocity.x = (finalDirX / fLen) * speed;
            def.body.velocity.z = (finalDirZ / fLen) * speed;
            def.mesh.rotation.y = Math.atan2(finalDirX, finalDirZ);
            def.body.position.y = DEFENDER_HALF_HEIGHT;
        } else {
            def.body.velocity.x = (dx / distXZ) * 4.0;
            def.body.velocity.z = (dz / distXZ) * 4.0;
            def.body.velocity.y = 5.0;
        }
        ...
        game.basketballBody.position.set(
            def.body.position.x + holdOffset.x,
            def.body.position.y + holdOffset.y,
            def.body.position.z + holdOffset.z
        );
        ...
        if (distXZ < 0.8 && def.body.position.y > 1.8) {
            def.body.position.y = DEFENDER_HALF_HEIGHT;
            def.body.velocity.set(0, 0, 0);
            this.defenderDunk(game, def);
        }
    }

    _handleStealAttempt(game, def, playerPos, delta, now) {
        this._moveDefenderTo(def, playerPos, delta, 0.75);
    }

    _moveDefenderTo(def, target, delta, speedMultiplier) {
        const dx = target.x - def.body.position.x;
        const dz = target.z - def.body.position.z;
        const dist = Math.sqrt(dx * dx + dz * dz);

        if (dist > 0.1) {
            const speed = MOVE_SPEED * speedMultiplier;
            def.body.velocity.x = (dx / dist) * speed;
            def.body.velocity.z = (dz / dist) * speed;
            def.mesh.rotation.y = Math.atan2(dx, dz);
        } else {
            def.body.velocity.set(0, 0, 0);
        }
    }
    ...
    _screenAndBlockPlayer(def, targetPos, delta) {
        const dx = targetPos.x - def.body.position.x;
        const dz = targetPos.z - def.body.position.z;
        const distXZ = Math.sqrt(dx * dx + dz * dz);
        const optimalBlockDist = 0.8;

        if (distXZ > optimalBlockDist) {
            const speed = MOVE_SPEED * 0.7;
            def.body.velocity.x = (dx / distXZ) * speed;
            def.body.velocity.z = (dz / distXZ) * speed;
            def.mesh.rotation.y = Math.atan2(dx, dz);
        } else {
            def.body.velocity.x = (dx / distXZ) * (MOVE_SPEED * 0.1);
            def.body.velocity.z = (dz / distXZ) * (MOVE_SPEED * 0.1);
            def.mesh.rotation.y = Math.atan2(dx, dz);
        }
        def.body.position.y = DEFENDER_HALF_HEIGHT;
    }
    ...
    handleStealSuccess(game, defender) {
        console.log("抢断成功！进入转换进攻！");
        game.basketballHandler.detachBall(game);
        this.defenderPickUpBall(game, defender);
        game.exclamationRedTimer = 0.5;
        defender.cooldown = 2.0;
        defender.continuousStealTime = 0;
        defender.stealHistory = [];

        for (const def of game.defenders) {
            if (def !== defender && def.cooldown <= 0) {
                def.cooldown = 1.5;
            }
        }
    }
    ...
    performPlayerSteal(game) {
        ...
        const dist = game.playerModel.position.distanceTo(def.body.position);

        if (dist <= game.playerStealRange) {
            console.log("%c[STEAL] 你成功从对手手中抢回了篮球！", "color: #00FF00; font-weight: bold;");
